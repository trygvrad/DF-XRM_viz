...
def set_backend(new_backend):
    ...
    sets the backend
    input:
        new_backend: string 'matplotlib' or 'mayavi'
    '''
    global backend
    backend = new_backend
    if backend == 'mayavi':
...
class Text:
    ...
    def rotate_z(self,phi):
        ...
        self.loc[0] = x
        self.loc[1] = z
    def transform(self,factor=15):
        '''
        generates 2d coorinates and sets colors
        '''
        if self.transformed==True:
            return
        self.xx=factor*(self.loc[0])/(-self.loc[2]+factor)
        self.yy=factor*(self.loc[1])/(-self.loc[2]+factor)
    ...
    def draw(self,ax):
        ...
        '''
        if backend == 'mayavi':
            mayavi.mlab.text3d(self.loc[0], self.loc[1], self.loc[2],self.text, opacity = self.color[3],  color=tuple(self.color[0:3]), scale=self.scale*3)
        elif backend == 'matplotlib':
            ax.text(self.xx,self.yy, self.text, fontsize=20*self.scale, color=self.color[0:3])
        elif backend == 'plotly':
            data = dict(
                    showarrow=False,
                    x=self.loc[0],
...
class NodeCollection:
    ...
    Class that maintains node collections
        member variables:
            ...
            transformed: bool, True if nodes have been transformed to 2d projection
            xx = x coordinate in 2d projection, generated by transform()
            yy = y coordinate in 2d projection, generated by transform()
        member functions:
            rotate_x: rotates around x axis (horisontal)
            rotate_y: rotates around y axis (vertical)
            rotate_z: rotates around z axis (out-of-plane)
    ...
    def transform(self,factor=15):
        if self.transformed==True:
            return
        self.xx=factor*(self.node_locations[:,0])/(-self.node_locations[:,2]+factor)
        self.yy=factor*(self.node_locations[:,1])/(-self.node_locations[:,2]+factor)
        self.transformed = True
    def rotate_x(self,phi):
        '''
        rotate_x: rotates around x axis (horisontal)
...
class QuadrilateralColormesh:
    '''
    projects a 2d colormesh forming a surface defined by four nodes.

    '''
    def __init__(self, node_collection, corner_nodes, image_data, opacity_data=None, colormap = 'viridis'):
        self.node_collection = node_collection
        self.corner_nodes = corner_nodes
        self.image_data = image_data
        self.opacity_data = opacity_data
        self.colormap = colormap
    def draw(self, ax):
        '''
            Renders the facet on the matplotlib ax
        '''
        if backend == 'mayavi':
            obj = mayavi.mlab.imshow(self.image_data, colormap=self.colormap, figure=ax)

            nodes = self.node_collection.node_locations[self.corner_nodes]
            v1 = nodes[1]-nodes[0]
            v2 = nodes[2]-nodes[1]
            normal = np.cross(v1,v2)

            v1= v1/np.sqrt(np.sum(v1**2))
            v3= normal/np.sqrt(np.sum(normal**2))
            v2 = np.cross(v1,v3)

            roll = -np.arcsin(v2[2])*180/np.pi
            yaw = 180 + np.arctan2(-v2[0], v2[1])*180/np.pi
            pitch = np.arctan2(v3[2], v1[2])*180/np.pi-90
            obj.actor.orientation = (roll,pitch,yaw)
            obj.actor.position = np.average(self.node_collection.node_locations[self.corner_nodes],axis=0)
            scale = np.sqrt(np.sum((nodes[1]-nodes[0])**2))/self.image_data.shape[0]
            scale2 = np.sqrt(np.sum((nodes[2]-nodes[1])**2))/self.image_data.shape[1]
            obj.actor.scale = [scale, scale2, 0]        # the required scale

        elif backend == 'matplotlib':
            n_xx = self.node_collection.xx[self.corner_nodes]
            n_yy = self.node_collection.yy[self.corner_nodes]

            shape = np.swapaxes(self.image_data,0,1).shape
            xx = np.zeros(shape)
            yy = np.zeros(shape)
            for i in range(shape[0]):
                y0 = n_yy[0] + (n_yy[3]-n_yy[0])*i/shape[0]
                y1 = n_yy[1] + (n_yy[2]-n_yy[1])*i/shape[0]
                yy[i,:] = np.arange(shape[1])/shape[1]*(y1-y0)+y0
                x0 = n_xx[0] + (n_xx[3]-n_xx[0])*i/shape[0]
                x1 = n_xx[1] + (n_xx[2]-n_xx[1])*i/shape[0]
                xx[i,:] = np.arange(shape[1])/shape[1]*(x1-x0)+x0
            s = ax.pcolormesh(xx,yy,np.swapaxes(self.image_data,0,1), cmap=self.colormap)
            if hasattr(self,'z_order'):
                s.set_zorder(self.z_order)

    def transform(self,factor=15):
        '''
        generates 2d coorinates and sets colors
        '''
        self.node_collection.transform(factor=factor)
        self.loc=np.average(self.node_collection.node_locations[self.corner_nodes], axis=0)

    def to_dict(self):

        dic = {
            'type':'QuadrilateralColormesh',
            'span':self.node_collection.node_locations[self.corner_nodes],
            'image_data':self.image_data,
            'colormap':'viridis',
            'opacity_data':self.opacity_data,
        }
        return dic

class Mesh(NodeCollection):
    '''
    A 3d traingle mesh object to render the mesh in matplotlib
    ...
    def draw(self,ax):
        '''
            Renders the trainglemesh on the matplotlib ax
        '''
        ...
        if backend == 'mayavi':
            if not self.abscolor:
                ...
                mesh = mayavi.mlab.triangular_mesh(self.node_locations[:,0],self.node_locations[:,1],self.node_locations[:,2],self.faces,
                                            representation='wireframe',
                                            opacity=0, reset_zoom = False)
                # add colors
                cell_data = mesh.mlab_source.dataset.cell_data
                from tvtk.api import tvtk
                sc=tvtk.UnsignedCharArray()
                sc.from_array(colors[:,:4]*255)
                #print(colors[:,:4]*255)
                cell_data.scalars = sc
                cell_data.scalars.name = 'Cell data'
                cell_data.update()
                mesh2 = mayavi.mlab.pipeline.set_active_attribute(mesh,
                        cell_scalars='Cell data')
                mesh3 = mayavi.mlab.pipeline.surface(mesh2)
                mesh3.actor.property.lighting = True
            else: # mayavi, abscolor
                mesh = mayavi.mlab.triangular_mesh(self.node_locations[:,0],self.node_locations[:,1],self.node_locations[:,2],self.faces,
                                            color = tuple(self.facecolor[0:3]),
                                            opacity = self.facecolor[3], reset_zoom = False)
                mesh.actor.property.lighting = True

        elif backend == 'matplotlib':
            xy = np.c_[self.xx,self.yy]
            verts = xy[self.faces]
            if not self.abscolor:
                pc = matplotlib.collections.PolyCollection(verts, edgecolors=self.meshcolor, facecolors=colors)
            else:
                pc = matplotlib.collections.PolyCollection(verts, edgecolors=self.meshcolor, facecolors=self.facecolor)
            s = ax.add_collection(pc)
            if hasattr(self,'z_order'):
                s.set_zorder(self.z_order)
        elif backend == 'plotly':
            if not self.abscolor:
...
class BoxFacet:
    ...
    def draw(self,ax):
        ...
        if backend == 'mayavi':
            ...
            x = np.array([[n[0,0],n[1,0]], [n[3,0],n[2,0]]])
            y = np.array([[n[0,1],n[1,1]], [n[3,1],n[2,1]]])
            z = np.array([[n[0,2],n[1,2]], [n[3,2],n[2,2]]])
            if not self.abscolor:
                color = self.get_color()
                mesh = mayavi.mlab.mesh( x,y,z, opacity = color[3], color = tuple(color[0:3]),  figure = ax)
            else:
                mesh = mayavi.mlab.mesh( x,y,z, opacity = self.facecolor[3], color = tuple(self.facecolor[0:3]),  figure = ax)
            mesh.actor.property.lighting = True
            #mayavi.mlab.triangular_mesh(self.node_locations[:,0],self.node_locations[:,1],self.node_locations[:,2],self.faces, color = tuple(self.facecolor[0:3]), opacity = self.facecolor[3])
        elif backend == 'matplotlib':
            xx = self.node_collection.xx[self.corner_nodes]
            yy = self.node_collection.yy[self.corner_nodes]
            if not self.abscolor:
                color = self.get_color()
                s = ax.fill(xx, yy, facecolor=color, lw=0)
            else:
                s = ax.fill(xx, yy, facecolor=self.facecolor, lw=0)
            if hasattr(self,'z_order'):
                s[0].set_zorder(self.z_order)
        elif backend == 'plotly':
            self.facecolor = np.array(self.facecolor)
            if not self.abscolor:
    ...
    def transform(self,factor=15):
        ...
        if not self.abscolor:
            normal = np.cross(   self.node_collection.node_locations[self.corner_nodes[1]] - self.node_collection.node_locations[self.corner_nodes[0]]
                                ,self.node_collection.node_locations[self.corner_nodes[1]] - self.node_collection.node_locations[self.corner_nodes[2]])
            normal = normal/np.sqrt(np.sum(normal**2))
            self.facecolor = get_color(self.facecolor, normal)
...
class BoxLine:
    ...
    def draw(self,ax):
        '''
            Renders the line on the matplotlib ax
        '''
        if backend == 'mayavi':
            locs = self.node_collection.node_locations[self.corner_nodes]
            lines = mayavi.mlab.plot3d(  locs[:,0], locs[:,1], locs[:,2],
                opacity = self.meshcolor[3], color = tuple(self.meshcolor[0:3]), figure = ax, tube_radius=self.linewidth*0.05,reset_zoom=False)
            lines.actor.property.lighting = True
            #mayavi.mlab.triangular_mesh(self.node_locations[:,0],self.node_locations[:,1],self.node_locations[:,2],self.faces, color = tuple(self.facecolor[0:3]), opacity = self.facecolor[3])
        elif backend == 'matplotlib':
            xx = self.node_collection.xx[self.corner_nodes]
            yy = self.node_collection.yy[self.corner_nodes]
            s = matplotlib.lines.Line2D(xx, yy, linestyle = '-', color=self.meshcolor, linewidth=self.linewidth,solid_capstyle='round')
            ax.add_line(s)
            if hasattr(self,'z_order'):
                s.set_zorder(self.z_order)
        elif backend == 'plotly':
    ...
    def transform(self,factor=15):
        '''
        Transforms from 3d to 2d coordinates for matplotlib. Factor is the location of camera defined as [0,0,-factor](?)
        '''
        self.node_collection.transform(factor=factor)
        self.loc=np.average(self.node_collection.node_locations[self.corner_nodes], axis=0)
...
class Atom:
    ...
    def transform(self, factor=15):
        ...
        self.xx = factor*(self.loc[0])/(-self.loc[2]+factor)
        self.yy = factor*(self.loc[1])/(-self.loc[2]+factor)
        self.radius = factor*self.radius/(-self.loc[2]+factor)
        self.factor = factor
        self.transformed = True
    ...
    def draw(self,ax):
        '''
            Renders the atom
        '''
        if backend == 'mayavi' or backend == 'plotly':
        ...
        if backend == 'mayavi':
            # create invisible mesh
            mesh = mayavi.mlab.triangular_mesh(vertices[:,0],vertices[:,1],vertices[:,2],isochahedron_faces,
                                        representation='wireframe',
                                        opacity=0, reset_zoom = False)
            #
            cell_data = mesh.mlab_source.dataset.cell_data
            ...
            center=[self.xx,self.yy]
            color=self.color
            # create base circle
            color=np.array(color)
            smolx,smoly=get_circ()
            basex=smolx*self.radius+center[0]
            basey=smoly*self.radius+center[1]
            ax.fill(basex,basey,color=self.color*0.8+np.array([0,0,0,0.2]))
            # create circle with cresent shape
            offset_center_xy = center + np.array([0.2,0.4])*self.radius
            for i,x in enumerate(basex):
                vec_point_to_offset = np.array([ basex[i]-offset_center_xy[0], basey[i]-offset_center_xy[1] ])
                dist = np.sqrt(np.sum(vec_point_to_offset**2))
                if dist > self.radius:
                    # move point to cresent
                    norm_vec = vec_point_to_offset/dist
                    basex[i] = offset_center_xy[0]+norm_vec[0]*self.radius
                    basey[i] = offset_center_xy[1]+norm_vec[1]*self.radius
            ax.fill(basex,basey,color=self.color)
            # create small highlight circle
            offset = np.array([0.2,0.4])*self.radius
            smolx,smoly=get_circ()
            basex=smolx*self.radius*0.4+center[0]+offset[0]
            basey=smoly*self.radius*0.4+center[1]+offset[1]
            ax.fill(basex,basey,color=self.color*0.8+np.array([0.2,0.2,0.2,0.2]))
        elif backend == 'plotly':
            data = plotly.graph_objects.Mesh3d(
                            x = vertices[:,0],
                            y = vertices[:,1],
